...
*/
            ...
            let point2 = column.drawOn(page, false)
/*
            let dim1 = column.getSize()
            let dim2 = column.getSize()
            let dim3 = column.getSize()
            print("height0: \(dim0.getHeight()))
            print("point1.x: \(point1.getX())    point1.y \(point1.getY()))
            print("point2.x: \(point2.getX())    point2.y \(point2.getY()))
...
*/
            ...
            column.setLocation(50.0, point2.getY() + 10.0)
            p = Paragraph()
            p.add(TextLine(font, "Peter Blood, bachelor of medicine and several other things besides, smoked a pipe and tended the geraniums boxed on the sill of his window above Water Lane in the town of Bridgewater."))
            column.addParagraph(p)
...
*/
            let box = Box()
            box.setLocation(point.getX(), point.getY() + 10.0)
